...
app = Flask(__name__)
configured_secret = os.environ.get("APP_SECRET_KEY")
if configured_secret:
    app.secret_key = configured_secret
else:
    # Safe for local development, but intentionally noisy because sessions do
    # not survive a restart. Production deployments must provide a stable key.
...
app.config.update(
    ...
    SESSION_COOKIE_SECURE=os.environ.get("APP_COOKIE_SECURE", "").strip().lower()
    in {"1", "true", "yes", "on"},
)
...
if __name__ == "__main__":
    app.run(debug=os.environ.get("FLASK_DEBUG", "").strip().lower() in {"1", "true", "yes", "on"})
