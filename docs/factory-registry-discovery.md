...
## مصرف رجیستری پس از Phase 6.8

کشف این سند همچنان import یک‌باره Phase 6.5A است و در request اجرا نمی‌شود.
`FactoryService` در همه ماژول‌های کارخانه‌محور فهرست canonical را از store
می‌گیرد؛ بنابراین کارخانه ساخته‌شده در Phase 6.7 بدون اسکن مجدد، restart یا
ویرایش کد منتشر می‌شود. `operational_key` فقط پس از اعتبارسنجی ID و مجوز در
backend برای دسترسی به پیکربندی قدیمی استفاده می‌شود. موجودی کامل surfaceها و
قواعد empty/inactive در `docs/factory-integration-inventory.md` آمده است.
